...
def _get_pooled_connection(**kwargs):
    ...
    try:
        ...
        raise InterfaceError("Failed getting connection from pool '{0}'".format(pool_name))
...
def _get_failover_connection(**kwargs):
    ...
    support_cnx_args = set(
        ["user", "password", "host", "port", "unix_socket", "database", "pool_name", "pool_size", "priority"]
    )
    ...
    for server in failover:
        ...
        if server["priority"] < 0 or server["priority"] > 100:
            raise InterfaceError(
                "Priority value should be in the range of 0 to 100, " "got : {}".format(server["priority"])
            )
        if not isinstance(server["priority"], int):
            raise InterfaceError(
                "Priority value should be an integer in the range of 0 to " "100, got : {}".format(server["priority"])
            )
...
def connect(*args, **kwargs):
    ...
    if dns_srv:
        ...
        if "unix_socket" in kwargs:
            raise InterfaceError("Using Unix domain sockets with DNS SRV " "lookup is not allowed")
        if "port" in kwargs:
            raise InterfaceError("Specifying a port number with DNS SRV " "lookup is not allowed")
        if "failover" in kwargs:
            raise InterfaceError("Specifying multiple hostnames with DNS " "SRV look up is not allowed")
        ...
        try:
            ...
            raise InterfaceError("Unable to locate any hosts for '{0}'" "".format(kwargs["host"]))
        ...
        kwargs["failover"] = [{"host": srv["host"], "port": srv["port"]} for srv in failover]
